...
type Props = {
  viewModel: CreateFilmHubViewModel;
  closeModal: () => void;
};

export const CreateFilmHub = observer(({ viewModel, closeModal }: Props) => {
  useEffect(() => {
    viewModel.reset();
  }, [viewModel]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 animate-fade-in">
      <div className="bg-white p-10 rounded-2xl shadow-2xl w-full max-w-lg relative animate-slide-in">
        ...
        <h2 className="text-2xl font-bold text-center text-gray-700 mb-4">Create Film</h2>
        {viewModel.error && <p className="text-red-500 text-center mt-2">{viewModel.error}</p>}
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); viewModel.doCreateFilmHub(); }}>
          <div>
